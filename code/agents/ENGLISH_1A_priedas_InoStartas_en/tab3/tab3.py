...
class EnhancedEUFormAgent:
    ...
    def create_enhanced_extraction_prompt(self, input_text: str, prompt_file_path: str = None) -> str:
        ...
        if prompt_file_path is None:
            prompt_file_path = os.path.join("prompts", "3.txt")
...
Examples:
  ...
  python enhanced_agent.py --input data.txt --template form.xlsx --api-key YOUR_KEY
    ...
    parser.add_argument(
        '--prompt-file', '-p',
        help='Path to custom prompt file (default: prompts/3.txt)'
    )
...
def main3(data_file_path):
    """Main function with hardcoded input/output paths for testing"""
    try:
       
        excel_template = "code/agents/ENGLISH_1A_priedas_InoStartas_en/tab3/input3.xlsx"
        output_file = "code/agents/ENGLISH_1A_priedas_InoStartas_en/tab3/3.xlsx"
        prompt_file = "code/prompts/3.txt"
