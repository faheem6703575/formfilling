...
class EnhancedEUFormAgent:
    ...
    def create_enhanced_extraction_prompt(self, input_text: str, prompt_file_path: str = None) -> str:
        ...
        if prompt_file_path is None:
            prompt_file_path = os.path.join("prompts", "2.txt")
...
Examples:
  ...
  python enhanced_agent.py --input data.txt --template form.xlsx --api-key YOUR_KEY
    ...
    parser.add_argument(
        '--prompt-file', '-p',
        help='Path to custom prompt file (default: prompts/2.txt)'
    )
...
def main2(data_file_path):
    """Main function with hardcoded input/output paths for testing"""
    try:
        
        excel_template = "code/agents/ENGLISH_1A_priedas_InoStartas_en/tab2/input2.xlsx"
        output_file = "code/agents/ENGLISH_1A_priedas_InoStartas_en/tab2/2.xlsx"
        prompt_file = "code/prompts/2.txt"
